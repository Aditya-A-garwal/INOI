...
void solve() {
	...
	int x[n + 2]{0}, y[n + n + 2], ans[n];
	int *diff = x + 1, *b = y + n, prev{0};
	...
	char c;
	for(int i = 0; i < n; i++)
		cin >> c, prev += c - '0', diff[i] = 2*prev - (i + 1), b[diff[i]] = i;

	for(int i = 0, target1, target2; i < n; ) {

		ans[i] = 1;
		target1 = diff[i - 1] + k, target2 = diff[i - 1] - k;

		if(abs(diff[n - 1] - diff[i - 1]) <= k) break;
		i = 1 + max(b[target1], b[target2]);
	}
